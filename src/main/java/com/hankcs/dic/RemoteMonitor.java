...
public class RemoteMonitor implements Runnable {
    ...
    private void runUnprivileged() {
        ...
        try (CloseableHttpResponse resp = httpclient.execute(head);) {
            if (resp.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                if ((resp.getLastHeader(HttpHeaders.LAST_MODIFIED) != null)
                        && !resp.getLastHeader(HttpHeaders.LAST_MODIFIED).getValue().equalsIgnoreCase(lastModified)) {
                    loadRemoteCustomWords(resp);
                } else if ((resp.getLastHeader(HttpHeaders.ETAG) != null)
                        && !resp.getLastHeader(HttpHeaders.ETAG).getValue().equalsIgnoreCase(eTags)) {
                    loadRemoteCustomWords(resp);
                }
            } else if (resp.getStatusLine().getStatusCode() == HttpStatus.SC_NOT_MODIFIED) {
                logger.info("remote_ext_dict {} is without modified", location);
            } else {
                logger.info("remote_ext_dict {} return bad code {}", location, resp.getStatusLine().getStatusCode());
            }
        } catch (Exception e) {
    ...
    private void loadRemoteCustomWords(CloseableHttpResponse response) {
        ...
        eTags = response.getLastHeader(HttpHeaders.ETAG) == null ? null :
                response.getLastHeader(HttpHeaders.ETAG).getValue();
    }
    ...
    private void loadRemoteStopWordsUnprivileged(String location) {
        ...
        get.setConfig(buildRequestConfig());
        try (CloseableHttpClient cli = HttpClients.createDefault();
             CloseableHttpResponse resp = cli.execute(get);) {
            if (resp.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                try (InputStreamReader isr = new InputStreamReader(resp.getEntity().getContent(), analysisDefaultCharset(resp));
                     BufferedReader in = new BufferedReader(isr)) {
                    String line;
                    boolean firstLine = true;
                    while ((line = in.readLine()) != null) {
